...
# guard to avoid running multiple instances of socat
if ! pgrep -f "socat UNIX-LISTEN:$HOME/.ssh/agent.sock,fork EXEC:$wsl2_ssh_pageant_bin" >/dev/null 2>&1; then
    ...
    if ! ss -a 2>/dev/null | grep -q "$GPG_AGENT_SOCK"; then
